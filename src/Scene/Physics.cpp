...
void Physics::check_tile_collision(Entity& a, Tilemap& tilemap)
{
	...
	auto f = [&a, d, &tilemap](int x, int y) {
		...
		if (t.solid) {
			...
			e.collider.width = e.collider.height = static_cast<float>(d);
	...
	for (int x = static_cast<int>(a.left()); x <= static_cast<int>(a.right()); x += static_cast<int>(a.right()) - static_cast<int>(a.left())) {
		for (int y = static_cast<int>(a.bottom()); y <= static_cast<int>(a.top()); y += d) {
			f(x / d, y / d);
		}
	}
	for (int x = static_cast<int>(a.left()) + d; x <= static_cast<int>(a.right()) - d; x += d) {
		for (int y = static_cast<int>(a.bottom()); y <= static_cast<int>(a.top()); y += static_cast<int>(a.top()) - static_cast<int>(a.bottom())) {
			f(x / d, y / d);
		}
	}
}
