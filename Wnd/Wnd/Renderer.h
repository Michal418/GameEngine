...
#define GAME_SUCCEEDED(game_result) (bool)( (int)game_result >= 0 )
#define GAME_FAILED(game_result) (bool)( (int)game_result < 0)

#define S_RETURN 0x80004008;

enum class GameResult { ok = 1, failed = -1, quit = -2 };
...
public:
	...
	bool process_messages(HRESULT& hr);
	HRESULT init(uint16_t width, uint16_t height);
