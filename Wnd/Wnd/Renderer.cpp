...
Renderer::Renderer(Scene* scene_ptr)
	: m_hwnd{}, msg{}, m_direct2d_factory_ptr(NULL), m_render_target_ptr(NULL), m_win_height{}, m_win_width{}
{
...
HRESULT Renderer::init(uint16_t width, uint16_t height)
{
	...
	for (Entity* entity : *m_scene_ptr->m_entity_collection_ptr)
	{
		if (m_texture_keys[entity->Id])
			continue;

		Sprite sprite{};
		sprite.size = entity->texture.size;
		if (SUCCEEDED(load_image(entity->texture.path, sprite.bitmap)))
		{
...
HRESULT Renderer::draw_entity(Entity& entity)
{
	...
	*/
	...
	D2D1_RECT_F r = D2D1::RectF(
		entity.position.x - bitmap_size.width * sprite.size - cam.left(),
		m_win_height - entity.position.y - bitmap_size.height * sprite.size + cam.bottom(),
		entity.position.x + bitmap_size.width * sprite.size - cam.left(),
		m_win_height - entity.position.y + bitmap_size.height * sprite.size + cam.bottom()
	);
	...
	m_render_target_ptr->DrawLine(
		{
			...
			m_win_height - entity.position.y + cam.bottom()
		},
		{
			entity.position.x + entity.velocity.x - cam.left(),
			m_win_height - entity.position.y - entity.velocity.y + cam.bottom()
		},
...
{
	Renderer* pThis = Renderer::get_this_ptr(hwnd, uMsg, lParam);
	...
	case WM_PAINT:
	{
		...
		pThis->m_hresult = pThis->on_render();
		if (FAILED(pThis->m_hresult))
			PostQuitMessage(0);
...
bool Renderer::process_messages(HRESULT& hr)
{
